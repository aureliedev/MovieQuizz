...
const Proposal = (props) => {
  ...
  useEffect(() => {
    ...
    props.actors.forEach((actor) => {
      ...
      if (random > 0.5) {
        ...
        const indexInMoviesList = props.movies
          .map((movie) => movie.name)
          .indexOf(actor.movies[index]);
        proposal.push({
          actor: actor.name,
  ...
  const answerFetch = (yes, actor, movie) => {
    fetch(
      "https://api.themoviedb.org/3/person/" +
        actor.id +
        "/movie_credits?api_key=eb553d0ae0d677efb0e511568c1b789a&language=en-US",
      {
        method: "GET",
        dataType: "JSON",
        headers: {
  ...
  /****** RENDU VISUEL ********/
  return proposalList.length > 0 ? (
    <>
      <div className="main-card-proposal">
        <h4>
          Est-ce que <b>{proposalList[proposalCount].actor}</b> a joué dans{" "}
          <b>{proposalList[proposalCount].movie}</b> ? 🎥
        </h4>
        <div className="section-card">
          <div className="img-container">
            <img
              src={
                ...
                proposalList[proposalCount].portrait
              }
              alt="actor"
            />
          </div>
          <div className="img-container">
            <img
              src={
                "http://image.tmdb.org/t/p/w185" +
                proposalList[proposalCount].poster
              }
              alt="movie"
            />
          </div>
        ...
        <div className="section-card">
          <button
            ...
            onClick={() =>
              answerFetch(
                true,
                props.actors[proposalCount],
                proposalList[proposalCount].movie
              )
            }
          >
            ✔️ OUI
